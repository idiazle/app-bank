...
const MovementsScreen = () => {
  const ITEM_PAGE = 15;
  const [itemsDsplayed, setItemsDisplayed] = useState<Transaction[]>(MOCK_TRANSACTIONS.slice(0, ITEM_PAGE));
  const [page, setPage] = useState(1);

    const loadMore = () => {
    const nextPage = page + 1;
    const start = page * ITEM_PAGE;
    const end = start + ITEM_PAGE;
    const nextItems = MOCK_TRANSACTIONS.slice(start, end);
    if (nextItems.length > 0) {
      setItemsDisplayed(prev => [...prev, ...nextItems]);
      setPage(nextPage);
    }
  };
